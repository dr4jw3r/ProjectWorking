<?php 
	...
	class LanguageSpokenService implements IService{

		private $repo;

		/*::CONSTRUCTOR::*/
		public function __construct(){
			$this->repo = new Repository('LanguageSpoken');
		}
		...
		public function byProperty($params){
			...
			if($data['status'] == 1 && $data['count'] > 0){
				$data['data'] = Mapping::toObject('LanguageSpoken', $data['data']);
				return $data['data'];
			}
			else{
				return null;
		...
		public function saveOrUpdate($details){
			...
			if($language == null){
				...
				return $this->repo->compositeUpdate($details, array('user_id', 'language_code'));
			}
		}
